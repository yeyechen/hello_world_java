...
public class Lecture18 {
   ...
   *
   * 动态规划概括：
   *    1. 四种递归常见模型：从左往右尝试模型(当前index要或者不要)；
   *                      范围尝试模型；
   *                      样本对应模型；
   *                      业务限制模型。
   *    2. 分析有没有重复解
   *    3. 记忆化搜索(傻缓存)->严格表依赖动态规划 (如果没有枚举行为，两者同为最优解；如果有，表结构可以进行斜率优化)
   *    4. 尝试继续优化
   *
   * 面试中设计暴力递归过程的原则：
   *    1. 每一个可变参数的类型，一定不要比int类型更复杂
   *    2. 如果可变参数一定要比int更复杂，限制可变参数数量为1个(然后使用记忆化搜索)
   *    3. 可变参数的个数能少则少
   *
   * 暴力递归到动态规划的套路：
   *    1. 已经找到了一个不违反原则的暴力递归，并且的确存在解的重复调用
   *    2. 找到哪些参数的变化会影响返回值，对每一个列出变化范围
   *    3. 参数间的所有的组合数量，意味着表的大小
   *    4. 记忆化搜索的方法就是傻缓存，非常容易得到
   *    5. 规定好严格表的大小，分析位置的依赖顺序，然后从基础位置填写到最终解的位置
   *    6. 对于有枚举行为的决策过程，根据斜率优化进一步优化
   *
   * 动态规划的进一步优化：
   *    1. 空间压缩
   *    2. 状态化简
